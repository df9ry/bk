...
typedef void (*resp_f)(void* client_ctx, const char* head, const uint8_t* p_body, size_t c_body);

// Any session might offer this interface:
struct session_t {
    bk_error_t (*get) (void* session_ctx, const char* head, resp_f fun, void* ctx);
    bk_error_t (*post)(void* session_ctx, const char* head, const uint8_t* p_body, size_t c_body);
};
